...
setwd("/Users/aru/Desktop/Santander/input")
...
#Look at the violin plots seems that their is no relationship between the target with the index of the train dataframe. This is more dominated by the zero targets then for the ones.

#Let us see distribution of train attributes from 3 to 102
...
#Let us see distribution of test attributes from 102 to 201
...
ggplot()+#Distribution of sd values per column in train data
geom_density(aes(x=train_kurtosis),kernel='gaussian',show.legend=TRUE,color='blue')+theme_classic()+
#Distribution of sd values per column in test data
geom_density(aes(x=test_kurtosis),kernel='gaussian',show.legend=TRUE,color='red')+
...
#But,lightgbm is performing well on imbalanced data compared to other models based on scores of roc_auc_score.
